...
- `R`
- R packages `abind`, `collapse`, `data.table`, `foreach`, `lpjmlkit`, `ncdf4`,
  `stringi`, `terra`, `units`
...
- multi_cropping_suitability_GAEZ.R: Script calculates multicropping suitability
  based on agro-climatic resources from GAEZ version 3
- multi_cropping_suitability_GAEZ_v4.R: Script calculates multicropping
  suitability based on agro-climatic resources from GAEZ version 4
- read_AQUASTAT_legacy.R: Script reads data downloaded from AQUASTAT database in
  legacy format (used for original LandInG release) and converts format for
  further use. This script can be run in interactive or
  non-interactive mode to deal with values with additional metadata. It does not
  work for current downloads from the AQUASTAT data portal.
...
- split_global_harvested_areas_into_rainfed_irrigated.R: Script combines MIRCA
  ...
  the country scale
- split_global_harvested_areas_into_rainfed_irrigated_SLURM.sh: Example batch
  script to run split_global_harvested_areas_into_rainfed_irrigated.R as a job
  on the PIK cluster (accounting for additional module requirements)
...
- Generate a timeseries of gridded crop-specific harvested areas
  ...
  4. `read_AQUASTAT_legacy.R` (does not require steps 1 to 3)
  5. `multi_cropping_suitability_GAEZ.R` or
    `multi_cropping_suitability_GAEZ_v4.R` (does not require steps 1 to 4)
