...
cft_output_period <- LandInG_setup$landuse$output_period
...
cft_gridname <- stop("ENTER_PATH_TO_GRIDFILE")
...
cft_cowname <- stop("ENTER_PATH_TO_COUNTRY_FILE")
...
cft_cowmetaname <- stop("ENTER_PATH_TO_COUNTRY_META_FILE_OR_EMPTY_STRING")
...
cft_mapping <- list(
  "rainfed temperate cereals" = list(
    ...
    variable = LandInG_setup$landuse$rainfed_output_name,
  ...
  "rainfed pasture/managed grass" = list(
    source = "hyde_grazing",
    variable = LandInG_setup$landuse$hyde_grazing_varname
  ...
  "irrigated temperate cereals" = list(
    source = "ha_timeseries",
    variable = LandInG_setup$landuse$irrigated_output_name,
    band = "temperate cereals"
...
# Consistency check
if (!length(grep(fallow_name, cft_bands)) %in% c(0, 2)) {
  stop(
...
if (cft_include_cropland || cft_separate_cropland) {
  for (irr in c("rainfed", "irrigated")) {
    if (
      !any(grepl(paste(irr, cropland_name), cft_bands)) &&
    ...
    if (is.null(cft_mapping[[paste0(irr, cropland_name)]])) {
      cft_mapping[[paste(irr, cropland_name)]] <- list(
        ...
        variable = LandInG_setup$landuse[[paste0("hyde_", irr, "_varname")]]
      )
    }
  }
...
# Consistency check
if (!length(grep(cropland_name, cft_bands)) %in% c(0, 2)) {
  stop(
...
if (
  !all(regmatches(cft_bands, gregexpr("^[a-z]*", cft_bands)) %in%
    c("rainfed", "irrigated"))
) {
...
pattern <- paste0(
  basename(LandInG_setup$landuse$aggregated_timeseries_filename_base),
...
if (length(ha_timeseries_filenames) < 1) {
  stop(
    ...
    sQuote(dirname(LandInG_setup$landuse$aggregated_timeseries_filename_base)),
...
ha_bands <- which(
  sapply(
    ...
    function(indata, search) {
      length(indata) > 0 && indata$source == search
    },
...
ha_timeseries_vars_req <- unique(
  sapply(
    ...
    function(indata) indata$variable
  )
)
...
for (filename in ha_timeseries_filenames) {
  ha_timeseries_files[[filename]] <- ncdf4::nc_open(filename)
  ...
  for (v in ha_timeseries_vars_req) {
    ...
    if (varunit$hasatt) {
      ha_timeseries_is_fractional <- (
        ha_timeseries_is_fractional ||
          !units::ud_are_convertible(varunit$value, "m2")
      )
    } else {
      # Close all open NetCDF files
      invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
      stop(
        ...
        sQuote(file.path(LandInG_setup$landuse$working_dir, filename)),
...
if (
  min(cft_output_period) < min(ha_timeseries_startyears) ||
...
ha_timeseries_raster <- terra::rast(
...
## 2) hyde_grazing                                                            ##
if ("hyde_grazing" %in% sapply(cft_mapping, function(indata) indata$source)) {
  # NetCDF variables that must be present in files
  grazing_bands <- which(
    sapply(
      cft_mapping,
      function(indata, search) length(indata) > 0 && indata$source == search,
      search = "hyde_grazing"
    )
  )
  hyde_grazing_vars_req <- unique(
    sapply(cft_mapping[grazing_bands], function(indata) indata$variable)
  )
  ...
  hyde_grazing_startyears <- hyde_grazing_endyears <-
    rep(NA, length(LandInG_setup$landuse$hyde_grazing_filename))
  names(hyde_grazing_startyears) <- names(hyde_grazing_endyears) <-
    LandInG_setup$landuse$hyde_grazing_filename
  for (filename in LandInG_setup$landuse$hyde_grazing_filename) {
    hyde_grazing_files[[filename]] <- ncdf4::nc_open(filename)
    missing <- setdiff(
    ...
    if (length(missing) > 0) {
      ...
      # Close all open NetCDF files
      invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
      invisible(sapply(hyde_grazing_files, ncdf4::nc_close))
    ...
    for (v in hyde_grazing_vars_req) {
      varunit <- ncdf4::ncatt_get(hyde_grazing_files[[filename]], v, "units")
      if (varunit$hasatt) {
  ...
  hyde_grazing_raster <- terra::rast(
  ...
  hyde_grazing_area <- load_hyde_area(
    ...
    fileunits = LandInG_setup$landuse$hyde_area_file_units,
    return_units = LandInG_setup$landuse$fao_area_units,
    return_raster = hyde_grazing_raster,
    earth_radius = LandInG_setup$earthradius,
    gextent = global_extent
  )
  ...
  if (
    min(cft_output_period) < min(hyde_grazing_startyears) ||
...
for (irr in c("rainfed", "irrigated")) {
  ...
  hydevar_req <- unique(
    c(
      sapply(cft_mapping[cropland_bands], function(indata) indata$variable),
      LandInG_setup$landuse[[paste0("hyde_", irr, "_varname")]]
    )
  )
  ...
  hydevar_startyears <- hydevar_endyears <- rep(
    NA, length(LandInG_setup$landuse[[paste0("hyde_", irr, "_filename")]])
  )
  names(hydevar_startyears) <- names(hydevar_endyears) <-
    LandInG_setup$landuse[[paste0("hyde_", irr, "_filename")]]
  hydevar_files <- list()
  hydevar_is_fractional <- FALSE
  for (filename in names(hydevar_startyears)) {
    hydevar_files[[filename]] <- ncdf4::nc_open(filename)
    if (any(!hydevar_req %in% names(hydevar_files[[filename]]$var))) {
      ...
      # Close all open NetCDF files
      invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
      invisible(sapply(hydevar_files, ncdf4::nc_close))
    ...
    for (v in hydevar_req) {
      varunit <- ncdf4::ncatt_get(hydevar_files[[filename]], v, "units")
      if (varunit$hasatt) {
  ...
  if (
    min(cft_output_period) < min(hydevar_startyears) ||
  ...
  assign(
    ...
    terra::rast(hydevar_files[[filename]]$filename, subds = v, lyrs = 1)
  )
...
cft_gridheader <- lpjmlkit::read_header(cft_gridname)
cft_griddata <- lpjmlkit::read_grid(cft_gridname, silent = TRUE)$data
# Create raster object corresponding to grid data
cft_extent <- terra::ext(
  min(cft_griddata[, "lon"] - cft_gridheader$header["cellsize_lon"] / 2),
  max(cft_griddata[, "lon"] + cft_gridheader$header["cellsize_lon"] / 2),
  min(cft_griddata[, "lat"] - cft_gridheader$header["cellsize_lat"] / 2),
  max(cft_griddata[, "lat"] + cft_gridheader$header["cellsize_lat"] / 2)
)
...
cft_raster_gridindex <- terra::cellFromXY(cft_raster, cft_griddata)
cat(
...
# Country file
cft_cowheader <- lpjmlkit::read_header(cft_cowname)
if (cft_cowheader$header["ncell"] != cft_gridheader$header["ncell"]) {
  invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
  invisible(sapply(hyde_rainfed_files, ncdf4::nc_close))
  invisible(sapply(hyde_irrigated_files, ncdf4::nc_close))
  if (exists("hyde_grazing_files")) {
    invisible(sapply(hyde_grazing_files, ncdf4::nc_close))
  }
  stop(
...
cft_cowdata <- lpjmlkit::read_io(cft_cowname, silent = TRUE)$data
# read_io always returns data with time dimension even though cft_cowdata should
# only have 1 year. Remove time dimension.
dim(cft_cowdata) <- dim(cft_cowdata)[-grep("time", names(dim(cft_cowdata)))]
# Corresponding country names
if (
  exists("cft_cowmetaname") && nchar(cft_cowmetaname) > 0 &&
...
for (check in seq_along(source_check)) {
  ...
  cft2source <- round(terra::res(cft_raster) / terra::res(source_raster), 6)
  ...
  # Spatial extent
  if (
    (terra::xmin(cft_raster) - terra::xmin(source_raster)) <
      (-terra::xres(cft_raster) / 100)
  ) {
    invalid <- which(
      (cft_griddata[, "lon"] - terra::xres(cft_raster) / 2 -
         terra::xmin(source_raster)
      ) < (-terra::xres(cft_raster) / 100)
    )
    invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
    invisible(sapply(hyde_rainfed_files, ncdf4::nc_close))
  ...
  if (
    (terra::xmax(cft_raster) - terra::xmax(source_raster)) >
      (terra::xres(cft_raster) / 100)
  ) {
    invalid <- which(
      (cft_griddata[, "lon"] + terra::xres(cft_raster) / 2 -
         terra::xmax(source_raster)
      ) > (terra::xres(cft_raster) / 100)
    )
    invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
    invisible(sapply(hyde_rainfed_files, ncdf4::nc_close))
  ...
  if (
    (terra::ymin(cft_raster) - terra::ymin(source_raster)) <
      (-terra::yres(cft_raster) / 100)
  ) {
    invalid <- which(
      (cft_griddata[, "lat"] - terra::yres(cft_raster) / 2 -
         terra::ymin(source_raster)
      ) < (-terra::yres(cft_raster) / 100)
    )
    invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
    invisible(sapply(hyde_rainfed_files, ncdf4::nc_close))
  ...
  if (
    (terra::ymax(cft_raster) - terra::ymax(source_raster)) >
      (terra::yres(cft_raster) / 100)
  ) {
    invalid <- which(
      (cft_griddata[, "lat"] + terra::yres(cft_raster) / 2 -
         terra::ymax(source_raster)
      ) > (terra::yres(cft_raster) / 100)
    )
    invisible(sapply(ha_timeseries_files, ncdf4::nc_close))
    invisible(sapply(hyde_rainfed_files, ncdf4::nc_close))
  ...
  xborder <- abs(terra::xmin(cft_raster) - terra::xmin(source_raster))
  min_xres <- min(terra::xres(cft_raster), terra::xres(source_raster))
  yborder <- abs(terra::ymin(cft_raster) - terra::ymin(source_raster))
  min_yres <- min(terra::yres(cft_raster), terra::yres(source_raster))
  if (
    round(xborder / min_xres, 3) %% 1 != 0 ||
...
cft_is_fraction <- !units::ud_are_convertible(cft_output_units, "m2")
cft_gridarea <- lpjmlkit::calc_cellarea(
  cft_griddata[, "lat"],
  cft_gridheader$header["cellsize_lon"],
  cft_gridheader$header["cellsize_lat"],
  earth_radius = LandInG_setup$earthradius,
  return_unit = "m2"
) * units::ud_convert(1, "m2", LandInG_setup$landuse$fao_area_units)
...
cft_output_header <- lpjmlkit::create_header(
  name = "LPJLUSE",
  version = 3,
  order = 1,
...
lpjmlkit::write_header(cft_output_filename, cft_output_header, overwrite = TRUE)
...
for (year in seq(min(cft_output_period), max(cft_output_period))) {
  ...
  if (exists("hyde_grazing_raster")) {
    hyde_grazing_yeardata <- load_hyde_yeardata(
      ...
      target_unit = LandInG_setup$landuse$fao_area_units,
  ...
  for (irr in c("rainfed", "irrigated")) {
    ...
    for (cft in grep(paste0("^", irr), cft_bands, value = TRUE)) {
      ...
      if (cft_mapping[[cft]]$source == "hyde_grazing") {
        ...
        missing <- which(
          !cft_mapping[[cft]]$band %in% dimnames(ha_timeseries_yeardata)[[2]]
        )
    ...
    v <- LandInG_setup$landuse[[paste0(hydevar, "_varname")]]
    tmpcropland <- rowSums(
      array(
        hydevar_yeardata[cft_raster_gridindex, v],
        dim = c(length(cft_raster_gridindex), length(v))
        ...
        which(
          sapply(
            ...
            function(indata, source) {
              length(indata) > 0 && indata$source == source
            },
            source = "fallow"
          )
        )
  ...
  if (scale_down_cfts2cropland && any(tmpsum > (cft_gridarea * 1.0001))) {
    message(
      ...
      " cells by up to ",
      max(tmpsum - cft_gridarea), " ", LandInG_setup$landuse$fao_area_units
    )
  }
  if (cft_separate_cropland) {
  ...
  if (cft_is_fraction) {
    cat(
      ...
      sQuote(LandInG_setup$landuse$fao_area_units),
      "to",
    ...
    for (cft in dimnames(cft_output_yeardata)[[1]]) {
      cft_output_yeardata[cft, ] <- cft_output_yeardata[cft, ] /
        cft_gridarea * units::ud_convert(1, "", cft_output_units)
    ...
    cft_output_yeardata <- cft_output_yeardata * units::ud_convert(
      1,
      LandInG_setup$landuse$fao_area_units,
      cft_output_units
    )
    if (cft_separate_fallow) {
      ...
      cft_fallow_yeardata <- cft_fallow_yeardata * units::ud_convert(
        1,
        LandInG_setup$landuse$fao_area_units,
        cft_output_units
      )
    ...
    if (cft_separate_cropland) {
      ...
      cft_cropland_yeardata <- cft_cropland_yeardata * units::ud_convert(
        1,
        LandInG_setup$landuse$fao_area_units,
        cft_output_units
      )
  ...
  if (typeof(lpjmlkit::get_datatype(cft_output_header)$type) == "double") {
    writeBin(
      as.double(cft_output_yeardata / cft_output_header$header["scalar"]),
      cft_output_file,
      size = lpjmlkit::get_datatype(cft_output_header)$size,
      endian = cft_output_header$endian
    )
  } else if (
    typeof(lpjmlkit::get_datatype(cft_output_header)$type) == "integer"
  ) {
    if (year == min(cft_output_period)) {
      # Print message about reduced accuracy only once
      warning(
        "CFT data will be converted to ",
        typeof(lpjmlkit::get_datatype(cft_output_header)$type),
        " size ", lpjmlkit::get_datatype(cft_output_header)$size,
  ...
  if (cft_separate_cropland) {
    if (typeof(lpjmlkit::get_datatype(cft_cropland_header)$type) == "double") {
      writeBin(
        as.double(cft_cropland_yeardata / cft_cropland_header$header["scalar"]),
        cft_cropland_file,
        size = lpjmlkit::get_datatype(cft_cropland_header)$size,
        endian = cft_cropland_header$endian
      )
    } else if (
      typeof(lpjmlkit::get_datatype(cft_cropland_header)$type) == "integer"
    ) {
      if (year == min(cft_output_period)) {
        # Print message about reduced accuracy only once.
        warning(
          "cropland data will be converted to ",
          typeof(lpjmlkit::get_datatype(cft_cropland_header)$type),
          " size ", lpjmlkit::get_datatype(cft_cropland_header)$size,
  ...
  if (cft_separate_fallow) {
    if (typeof(lpjmlkit::get_datatype(cft_fallow_header)$type) == "double") {
      writeBin(
        as.double(cft_fallow_yeardata / cft_fallow_header$header["scalar"]),
        cft_fallow_file,
        size = lpjmlkit::get_datatype(cft_fallow_header)$size,
        endian = cft_fallow_header$endian
      )
    } else if (
      typeof(lpjmlkit::get_datatype(cft_fallow_header)$type) == "integer"
    ) {
      if (year == min(cft_output_period)) {
        # Print message about reduced accuracy only once.
        warning(
          "fallow land data will be converted to ",
          typeof(lpjmlkit::get_datatype(cft_fallow_header)$type),
          " size ", lpjmlkit::get_datatype(cft_fallow_header)$size,
...
size <- prod(cft_output_header$header[c("ncell", "nbands", "nyear")]) *
  lpjmlkit::get_datatype(cft_output_header)$size +
  lpjmlkit::get_headersize(cft_output_header)
if (file.size(cft_output_filename) != size) {
  warning(
    "File size appears to be wrong: ",
...
if (cft_separate_cropland) {
  ...
  size <- prod(cft_cropland_header$header[c("ncell", "nbands", "nyear")]) *
    lpjmlkit::get_datatype(cft_cropland_header)$size +
    lpjmlkit::get_headersize(cft_cropland_header)
...
if (cft_separate_fallow) {
  ...
  size <- prod(cft_fallow_header$header[c("ncell", "nbands", "nyear")]) *
    lpjmlkit::get_datatype(cft_fallow_header)$size +
    lpjmlkit::get_headersize(cft_fallow_header)
