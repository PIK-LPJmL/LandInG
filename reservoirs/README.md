...
be downloaded from: https://www.globaldamwatch.org/grand/
...
- `R`
- R packages `foreign`, `geosphere`, `lpjmlkit`, `lwgeom`
  (depending on version of `sf`), `maps`, `RColorBrewer`, `sf`, `terra`, `units`
...
- reservoir\_`[[version_string]]_[[resolution_string]]`_par`[[index of parameter combination]]`.bin:
  input file in LPJmL format
- reservoir\_`[[version_string]]_[[resolution_string]]`\_par`[[index of parameter combination]]`\_settings.csv:
  CSV table listing parameter values used to create matching input file
- reservoir_diagnostics\_`[[version_string]]_[[resolution_string]]_[[number of parameter combinations]]`_settings.pdf:
  maps showing GRanD dam and reservoir locations and assigned grid cells for
  all parameter combinations
- reservoir_diagnostics\_`[[version_string]]_[[resolution_string]]_[[number of parameter combinations]]`_settings.Rdata:
