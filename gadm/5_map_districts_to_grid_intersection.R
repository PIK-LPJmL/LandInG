...
################################################################################
## Confirm that directory with shapes per country exists (created in step 4). ##
if (!file.exists(split_directory_districts)) {
  stop(
    "split_directory_districts ",
    split_directory_districts,
    " not found. Please run script for step 4 first."
  )
}
################################################################################

################################################################################
## Find countries in split_directory_districts (defined in gadm_setup,r).     ##
country_shapes <- list.files(split_directory_districts, ".shp")
...
if (!file.exists(intersect_directory_districts)) {
  dir.create(intersect_directory_districts)
}
################################################################################

cat(
  "Country shapes in", split_directory_districts,
  "will be intersected with grid cell polygons in", gridcell_shapefile, "\n"
)

################################################################################
## Check if any result files exist already in intersect_directory_districts.  ##
## These will not be processed again. Delete them to run full intersection    ##
## again.                                                                     ##
...
if (length(country_shapes) < 1) {
  ...
  cat(
    ...
    "countries/regions/counties and", ncell(lpjgrid_raster), "grid cells.",
    "Results saved to:", intersect_directory_districts, "\n"
  )
}
################################################################################
...
################################################################################
## Check if grid cell shapefile exists. This has been created in step 1 or 4. ##
if (!file.exists(gridcell_shapefile)) {
  stop(
    "gridcell_shapefile ", gridcell_shapefile, " does not exist.\n",
    "Please run script for step 4 first."
  )
}
################################################################################
...
intersection_loop <- foreach(
  ...
  country_shape <- st_read(
    file.path(split_directory_districts, country_shapes[cindex]),
    quiet = TRUE
  )
  ...
  country_bbox[low] <- country_bbox[low] - max(lpj_res) * 2
  # 2. upper boundary
  up <- grep("max", names(country_bbox))
  country_bbox[up] <- country_bbox[up] + max(lpj_res) * 2
  # Crop to bounding box
  lpjgrid_shape_country <- st_crop(lpjgrid_shape, country_bbox)

  invalid_grid_warn <- ""
  if (any(!st_is_valid(lpjgrid_shape_country))) {
    invalid_grid_warn <- paste(
      "The cropped grid cell shape for", country_shapes[cindex], "has",
      length(which(!st_is_valid(lpjgrid_shape_country))),
      "invalid polygons. Trying to fix"
    )
    warning(invalid_grid_warn, call. = FALSE, immediate. = TRUE)
    lpjgrid_shape_country <- st_make_valid(lpjgrid_shape_country)
  }

  # Run intersection between grid cell shape and country shape
  country_gridpolygons <- st_intersection(lpjgrid_shape_country, country_shape)
  ...
  # Save country result to intersect_directory_districts
  st_write(
    country_gridpolygons,
    dsn = file.path(intersect_directory_districts, country_shapes[cindex]),
    delete_dsn = TRUE,
    quiet = TRUE
  )
...
failed <- which(
  !file.exists(file.path(intersect_directory_districts, country_shapes))
)
if (length(failed) > 0) {
  message(
