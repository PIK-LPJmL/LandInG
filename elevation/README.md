...
The included script to download and resample ETOPO1 data requires the Generic
Mapping Tools (GMT). GMT is available for Windows, MacOS and Linux from
https://www.generic-mapping-tools.org/

You will need `R` and the R packages `lpjmlkit` and `terra` to run
`elevation_input.R`.
...
- `../R_env_PIK.sh` can be used to set up the software environment for R on
...
- Script elevation_input.R will extract elevation for all cells included in
  ...
  `terra` package in R and must be in the correct resolution already. This
...
- If necessary, update path to `landing_setup.R` which is usually located in
  the base directory of the toolbox.
