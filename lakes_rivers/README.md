...
and 2. LandInG currently only supports GLWD version 1, not the newly released
GLWD version 2.
...
- `R`
- R packages `foreach`, `lpjmlkit`, `lwgeom`, `sf`, `terra`
...
- `output_format`: Select either one of the native LPJmL input formats
  ("BIN", "RAW") or a raster format supported by the terra package (e.g. NetCDF
  or ASCII grid) for files created by this script
