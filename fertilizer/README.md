...
- `R`
- R packages `abind`, `collapse`, `foreach`, `lpjmlkit`, `ncdf4`, `R.utils`,
  `stringdist`, `stringi`, `terra`, `tools`, `units`
...
possible and aggregate spatially only in step 6, if requested LPJmL input
...
LandInG users with access to the PIK high-performance cluster can use the
included bash scripts `*_SLURM.sh` to submit processing tasks to the PIK cluster
